...
# 6. Read project documentation if it exists
...
**Brownfield awareness:** If `.linear_project.json` contains `"mode": "brownfield"`,
you are working on an EXISTING codebase. Before making any changes:
- Read CLAUDE.md and README for project conventions
- Match existing code patterns, naming conventions, and file structure
- Don't introduce new libraries when existing alternatives are already in use
- Follow the existing test framework and patterns
- Follow the project's commit message conventions (check git log)
